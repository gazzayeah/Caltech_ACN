...
torch.save(agent.state_dict(), "../image/agent_model.pt")

#agent.load_state_dict(torch.load('../image/agent_model.pt', map_location=torch.device('cpu')))
...
for i_episode in itertools.count(1):
    ...
    if i_episode > testing_num_days:
        datafilename = dataFilePath + "agent_{}.npy".format(ACTION)
        with open(datafilename, 'wb') as f:
            np.save(f, env.reward_vec[1:])
        break
    # Print episode iterating information
    print("Episode: (day) {}, total numsteps: {}, episode steps: {}, reward: {}".format(i_episode, total_numsteps,
                                                                                        episode_steps,
